...
def data_load(args): 
    ...
    if args.trte == "val":
        ...
        tr_txt = txt_src
    print("--------DataLoader-------")
    print("source_tr: %d, source_te: %d, test: %d." %(len(tr_txt), len(te_txt), len(txt_test)))
...
def train_source(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % 100 == 0:
            iter_time = time.time()
            log_str = "iter_num: %d, max_iter: %d, loss: %.4f, time: %.2f" % (iter_num, max_iter, classifier_loss.item(), (iter_time-start_time)/100)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            netB.train()
            netC.train()
    log_str = "save model in " + args.output_dir_src
    args.out_file.write(log_str+"\n")
    args.out_file.flush()
    print(log_str+"\n")
...
if __name__ == "__main__":
    ...
    args.output_dir_src = osp.join(args.output, 'uda', args.dset, names[args.s][0].upper())
    ...
    print("[LOG files in: %s]"%args.output_dir_src) 
